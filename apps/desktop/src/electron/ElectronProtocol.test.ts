...
describe("ElectronProtocol", () => {
  it("derives isolated production and development origins from distribution identity", () => {
    assert.equal(ElectronProtocol.getDesktopOrigin(false), "t3-code-orchestrator://app");
    assert.equal(ElectronProtocol.getDesktopOrigin(true), "t3-code-orchestrator-dev://app");
    assert.equal(ElectronProtocol.getDesktopUrl(false), "t3-code-orchestrator://app/");
  });
  ...
  it.effect("proxies the stable renderer origin to the current app server", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          yield* protocol.registerDesktopProtocol({
            scheme: "t3-code-orchestrator-dev",
          ...
          const response = yield* Effect.promise(() =>
            handler!(
              new Request("t3-code-orchestrator-dev://app/api/health?verbose=1", {
                headers: {
                  accept: "application/json",
                  origin: "t3-code-orchestrator-dev://app",
                  referer: "t3-code-orchestrator-dev://app/",
          ...
          assert.include(
            response.headers.get("content-security-policy") ?? "",
            "img-src 'self' t3-code-orchestrator-dev: blob: data: http: https:",
          );
          assert.include(
            response.headers.get("content-security-policy") ?? "",
            "font-src 'self' t3-code-orchestrator-dev: data:",
          );
        }),
      );
      ...
      assert.deepEqual(unhandleMock.mock.calls, [["t3-code-orchestrator-dev"]]);
    ...
    Effect.gen(function* () {
      ...
      const response = yield* Effect.scoped(
        Effect.gen(function* () {
          const protocol = yield* ElectronProtocol.ElectronProtocol;
          yield* protocol.registerDesktopProtocol({
            scheme: "t3-code-orchestrator",
            targetOrigin: new URL("http://127.0.0.1:3773/"),
            backendOrigin: new URL("http://127.0.0.1:3773/"),
            clerkFrontendApiHostname: undefined,
          });
          return yield* Effect.promise(() =>
            handler!(new Request("t3-code-orchestrator://other/")),
          );
        }),
      );
    ...
    Effect.gen(function* () {
      ...
      const response = yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          return yield* Effect.promise(() =>
            handler!(new Request("t3-code-orchestrator-dev://app/")),
          );
        }),
      );
  ...
  it.effect("preserves protocol registration failures", () =>
    Effect.gen(function* () {
      ...
      assert.equal(error.scheme, "t3-code-orchestrator-dev");
      assert.strictEqual(error.cause, cause);
      assert.equal(
        error.message,
        'Failed to register Electron protocol scheme "t3-code-orchestrator-dev".',
      );
    }).pipe(Effect.provide(ElectronProtocol.layer)),
  );
  ...
  it.effect("preserves protocol unregistration failures", () =>
    Effect.gen(function* () {
      ...
      if (exit._tag === "Failure") {
        ...
        assert.equal(error.scheme, "t3-code-orchestrator");
        assert.strictEqual(error.cause, cause);
        assert.equal(
          error.message,
          'Failed to unregister Electron protocol scheme "t3-code-orchestrator".',
        );
      }
    }).pipe(Effect.provide(ElectronProtocol.layer)),
  );
  ...
  it("keeps executable sources host-restricted while allowing runtime network resources", () => {
    ...
    assert.deepEqual(directives["media-src"], ["'self'", "t3-code-orchestrator:", "blob:"]);
    assert.deepEqual(directives["font-src"], ["'self'", "t3-code-orchestrator:", "data:"]);
  });
});
