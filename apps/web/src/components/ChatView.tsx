      ...
      setLocalDispatch((current) => {
        ...
        if (active) {
          return active.preparingWorktree === preparingWorktree &&
            active.submissionIntent === (options?.submissionIntent ?? active.submissionIntent)
            ? active
            : {
                ...active,
                preparingWorktree,
                submissionIntent: options?.submissionIntent ?? active.submissionIntent,
              };
        }
...
function ChatViewContent(props: ChatViewProps) {
  ...
  // A split pane owns the available width, so the usual viewport breakpoint
  // would render an inline auxiliary surface outside the pane. Keep the
  // upstream right-panel surfaces, but use the pane's presentation policy.
  const shouldUseRightPanelSheet = shouldUsePlanSidebarSheet;
  ...
  const openThreadPullRequest = useCallback(
    (number: number) => {
      if (!activeThreadRef || !supportsPullRequests) return;
  ...
  // partition (same shell and same capability gate) so the banner and the
  ...
  const compactDisabledReason = compactDisabled
    ? feedbackUploading
      ? "Finish sending feedback before compacting"
  ...
  const onSend = async (
    ...
    if (composerRef.current?.validateProviderInput(messageTextForSend) === false) {
  ...
  const rightPanelContent = activeThreadRef ? (
    activeRightPanelSurface?.kind === "preview" ? (
      ...
      <PersistentThreadTerminalPanel
        ...
        onCloseTerminal={requestClosePanelTerminal}
    ...
    >
        ...
        <div className="flex min-h-0 min-w-0 flex-1">
          ...
          >
            {isWorkspaceFileDragActive ? (
              <div
                className="pointer-events-none absolute inset-2 z-40 flex items-center justify-center rounded-2xl border-2 border-dashed border-primary/60 bg-primary/[0.035] text-sm font-medium text-foreground"
      ...
      {!shouldUseRightPanelSheet && rightPanelOpen && activeThreadRef ? (
        <RightPanelTabs
          ...
          onAddPullRequest={() => {
            if (linkedThreadPullRequest) openThreadPullRequest(linkedThreadPullRequest.number);
          }}
          ...
          pullRequestAvailable={supportsPullRequests && linkedThreadPullRequest !== null}
