...
describe("changeRequestAutoSettles", () => {
  ...
  it("does not settle on a terminal change request touched after the thread's latest event", () => {
  ...
  it("never re-settles a thread revived after the merge", () => {
    ...
    // A merge landing after the revival still does not settle this fork.
    expect(
      changeRequestAutoSettles(
  ...
  it("does not settle when the merge lands during an in-flight turn", () => {
  ...
  it("does not settle when timestamps are missing or malformed", () => {
...
describe("effectiveSettled", () => {
  ...
  it("ignores change-request state while preserving inactivity settlement", () => {
    ...
    expect(
      effectiveSettled(makeShell({ activityAt: FRESH }), {
        now: NOW,
