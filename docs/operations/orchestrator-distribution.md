...
The current fork release is **`0.0.37-orchestrator.2`**. Its base release is upstream T3 Code
`0.0.37`; the suffix identifies an Orchestrator build and must remain on every desktop, server,
...
| Renderer origin              | `t3-code-orchestrator://app`                         |
| Linux desktop entry          | `t3-code-orchestrator.desktop`                       |
...
| Locks                        | `~/.t3-code-orchestrator/userdata/locks`             |
...
vp run dist:desktop:dmg:arm64 --build-version 0.0.37-orchestrator.2
vp run dist:desktop:dmg:x64 --build-version 0.0.37-orchestrator.2
vp run dist:desktop:linux --build-version 0.0.37-orchestrator.2
vp run dist:desktop:win:x64 --build-version 0.0.37-orchestrator.2
vp run dist:desktop:win:arm64 --build-version 0.0.37-orchestrator.2
```
...
The v0.0.37 packaging flow stages platform-specific icon resources before the builder runs and
ships the Windows backend as a `resources/server.asar` sidecar with its required unpacked native
files. Keep those lifecycle improvements when reconciling upstream changes. Preview builds must
remain publish-free, and release builds must also remain publish-free until a verified Orchestrator
feed is deliberately configured.
