...
```text
t3-code-orchestrator-dev://app/
t3-code-orchestrator://app/
```

Local desktop development uses `t3-code-orchestrator-dev://app`, while packaged builds use
`t3-code-orchestrator://app`. Add the matching origin to each Clerk instance's Backend API
`allowed_origins` array as well. The development Clerk instance should only need
`t3-code-orchestrator-dev://app`; the production Clerk instance should only need
`t3-code-orchestrator://app`. `@clerk/electron` owns the native request adapter, encrypted Clerk
token persistence, external-browser OAuth transport, and callback delivery for initial sign-in and
...
curl -X PATCH https://api.clerk.com/v1/instance \
  ...
  -d '{"allowed_origins":["t3-code-orchestrator://app"]}'
```
...
The production macOS bundle ID is `dev.mateuslucas.t3code.orchestrator`. To enable native passkeys:

1. Create an explicit macOS App ID for `dev.mateuslucas.t3code.orchestrator` in the Apple Developer portal and enable
   **Associated Domains**.
...
4. Confirm Clerk serves `https://<frontend-api>/.well-known/apple-app-site-association` and that
   `webcredentials.apps` contains `<TEAM_ID>.dev.mateuslucas.t3code.orchestrator`.
...
```sh
codesign --verify --deep --strict "/Applications/T3 Code Orchestrator.app"
codesign -d --entitlements :- "/Applications/T3 Code Orchestrator.app"
```
